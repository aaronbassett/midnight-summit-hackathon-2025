 ...
 */
type ImpureCircuit<PS> = (context: CircuitContext<PS>, ...args: any[]) => CircuitResults<PS, any>;
/**
 * A collection of impure circuits defined in a contract.
 *
 * @typeParam PS - The private state modified by the contract witnesses.
 */
type ImpureCircuits<PS> = Record<string, ImpureCircuit<PS>>;
/**
 * A type representing a witness in a contract.
 *
 * @typeParam PS - The private state modified by the witness.
 */
type Witness<PS> = (context: WitnessContext<any, PS>, ...rest: any[]) => [PS, any];
/**
 ...
 */
interface Contract<PS = any, W extends Witnesses<PS> = Witnesses<PS>> {
    /**
 ...
 */
type ImpureCircuitId<C extends Contract = Contract> = keyof C['impureCircuits'] & string;
/**
 * Extracts the private state of a contract.
 *
 * @typeParam C The contract for which we would like the private state.
 */
type PrivateState<C extends Contract> = C extends Contract<infer PS> ? PS : never;
/**
 ...
 * @typeParam C The contract type for which we would like impure circuit IDs.
 */
declare const getImpureCircuitIds: <C extends Contract>(contract: C) => ImpureCircuitId<C>[];
/**
 * The parameter types of the circuits in a contract.
 */
type CircuitParameters<C extends Contract, K extends ImpureCircuitId<C>> = Parameters<C['impureCircuits'][K]> extends [CircuitContext<any>, ...infer A] ? A : never;
/**
 * The return types of the circuits in a contract.
 */
type CircuitReturnType<C extends Contract, K extends ImpureCircuitId<C>> = ReturnType<C['impureCircuits'][K]> extends CircuitResults<any, infer U> ? U : never;
/**
 * The parameter type of the public state constructor.
 */
type InitialStateParameters<C extends Contract> = Parameters<C['initialState']> extends [ConstructorContext<any>, ...infer A] ? A : never;
...
declare const TRANSACTION_TO_PROVE = "TransactionToProve";
declare const BALANCE_TRANSACTION_TO_PROVE = "BalanceTransactionToProve";
declare const NOTHING_TO_PROVE = "NothingToProve";
...
type ProvingRecipe<TTransaction> = TransactionToProve | BalanceTransactionToProve<TTransaction> | NothingToProve<TTransaction>;
...
type ProvenTransaction = Transaction<SignatureEnabled, Proof, PreBinding>;
/**
 * The configuration for the proof request to the proof provider.
 */
interface ProveTxConfig<K extends string> {
    /**
 ...
 */
interface ProofProvider<K extends string> {
    /**
 ...
 * @typeParam PS - Parameter indicating the private state type stored, sometimes a union of private state types.
 */
interface MidnightProviders<ICK extends ImpureCircuitId = ImpureCircuitId, PSI extends PrivateStateId = PrivateStateId, PS = any> {
    /**
