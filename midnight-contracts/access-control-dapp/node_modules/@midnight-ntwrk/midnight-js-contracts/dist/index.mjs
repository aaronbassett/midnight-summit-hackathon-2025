import { ContractState as ContractState$1, signatureVerifyingKey, QueryContext as QueryContext$1, CostModel, emptyZswapLocalState, decodeZswapLocalState, sampleSigningKey } from '@midnight-ntwrk/compact-runtime';
import { ZswapOutput, ZswapTransient, ZswapInput, ZswapOffer, VerifierKeyInsert, MaintenanceUpdate, signData, Intent, VerifierKeyRemove, ReplaceAuthority, ContractState, ContractCallPrototype, communicationCommitmentRandomness, ContractDeploy, StateValue, QueryContext, ChargedState, ContractOperationVersionedVerifierKey, ContractOperationVersion, ContractMaintenanceAuthority, partitionTranscripts, PreTranscript, LedgerParameters, LedgerState, WellFormedStrictness } from '@midnight-ntwrk/ledger-v6';
import { getNetworkId } from '@midnight-ntwrk/midnight-js-network-id';
import { parseCoinPublicKeyToHex, parseEncPublicKeyToHex, assertDefined, assertIsContractAddress, fromHex, ttlOneHour, assertUndefined, toHex } from '@midnight-ntwrk/midnight-js-utils';
import { Transaction, getImpureCircuitIds, SucceedEntirely } from '@midnight-ntwrk/midnight-js-types';
import fs from 'fs';
 ...
 * limitations under the License.
 */
const toLedgerContractState = (contractState) => ContractState.deserialize(contractState.serialize());
const fromLedgerContractState = (contractState) => ContractState$1.deserialize(contractState.serialize());
...
const createUnprovenLedgerDeployTx = (verifierKeys, sk, contractState, zswapLocalState, encryptionPublicKey) => {
    ...
    return [
        ...
        Transaction.fromParts(getNetworkId(), zswapStateToOffer(zswapLocalState, encryptionPublicKey), undefined, Intent.new(ttlOneHour()).addDeploy(contractDeploy))
    ];
};
const createUnprovenLedgerCallTx = (circuitId, contractAddress, initialContractState, zswapChainState, partitionedTranscript, privateTranscriptOutputs, input, output, nextZswapLocalState, encryptionPublicKey) => {
    ...
    return Transaction.fromParts(getNetworkId(), zswapStateToOffer(nextZswapLocalState, encryptionPublicKey, {
        contractAddress,
        zswapChainState
    }), undefined, Intent.new(ttlOneHour()).addCall(new ContractCallPrototype(contractAddress, circuitId, op, partitionedTranscript[0], partitionedTranscript[1], privateTranscriptOutputs, input, output, communicationCommitmentRandomness(), circuitId)));
};
// Utilities for creating single contract updates.
const replaceAuthority = (newAuthority, contractState) => new ReplaceAuthority(contractMaintenanceAuthority(newAuthority, contractState));
const removeVerifierKey = (operation) => new VerifierKeyRemove(operation, new ContractOperationVersion('v2'));
const insertVerifierKey = (operation, newVk) => new VerifierKeyInsert(operation, new ContractOperationVersionedVerifierKey('v2', newVk));
...
const unprovenTxFromContractUpdates = (contractAddress, updates, contractState, sk) => {
    ...
    return Transaction.fromParts(getNetworkId(), undefined, undefined, Intent.new(ttlOneHour()).addMaintenanceUpdate(signedMaintenanceUpdate));
};
...
const call = (options) => {
    ...
    const initialTxContext = new QueryContext$1(initialContractState.data, contractAddress);
...
async function createUnprovenDeployTx(providers, options) {
    ...
    return createUnprovenDeployTxFromVerifierKeys(verifierKeys, parseCoinPublicKeyToHex(providers.walletProvider.zswapSecretKeys.coinPublicKey, getNetworkId()), options, providers.walletProvider.zswapSecretKeys.encryptionPublicKey);
}
...
async function createUnprovenCallTx(providers, options) {
    ...
    if (hasPrivateStateId && hasPrivateStateProvider) {
        ...
        return createUnprovenCallTxFromInitialStates(createCallOptions(options, parseCoinPublicKeyToHex(providers.walletProvider.zswapSecretKeys.coinPublicKey, getNetworkId()), contractState, zswapChainState, privateState), providers.walletProvider.zswapSecretKeys.coinPublicKey, providers.walletProvider.zswapSecretKeys.encryptionPublicKey);
    }
    const { zswapChainState, contractState } = await getPublicStates(providers.publicDataProvider, options.contractAddress);
    return createUnprovenCallTxFromInitialStates(createCallOptions(options, parseCoinPublicKeyToHex(providers.walletProvider.zswapSecretKeys.coinPublicKey, getNetworkId()), contractState, zswapChainState), providers.walletProvider.zswapSecretKeys.coinPublicKey, providers.walletProvider.zswapSecretKeys.encryptionPublicKey);
}
 ...
 */
const verifierKeysEqual = (a, b) => a.length === b.length && toHex(a) === toHex(b);
/**
