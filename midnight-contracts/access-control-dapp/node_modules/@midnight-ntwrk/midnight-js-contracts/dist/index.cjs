...
var ledgerV6 = require('@midnight-ntwrk/ledger-v6');
...
recipient.is_left
    ? ledgerV6.ZswapOutput.new(coinInfo, segmentNumber, recipient.left, encryptionPublicKey)
    : ledgerV6.ZswapOutput.newContractOwned(coinInfo, segmentNumber, recipient.right);
...
const zswapStateToOffer = (zswapLocalState, encryptionPublicKey, addressAndChainStateTuple) => {
    ...
    zswapLocalState.inputs.forEach((qualifiedCoinInfo) => {
        ...
        if (unprovenOutput) {
            unprovenTransients.set(serializedCoinInfo, ledgerV6.ZswapTransient.newFromContractOwnedOutput(qualifiedCoinInfo, DEFAULT_SEGMENT_NUMBER, unprovenOutput));
            ...
            unprovenInputs.set(serializedCoinInfo, ledgerV6.ZswapInput.newContractOwned(qualifiedCoinInfo, DEFAULT_SEGMENT_NUMBER, addressAndChainStateTuple.contractAddress, addressAndChainStateTuple.zswapChainState));
        }
    });
    const inputsOffer = unprovenOfferFromMap(unprovenInputs, ledgerV6.ZswapOffer.fromInput);
    const outputsOffer = unprovenOfferFromMap(unprovenOutputs, ledgerV6.ZswapOffer.fromOutput);
    const transientsOffer = unprovenOfferFromMap(unprovenTransients, ledgerV6.ZswapOffer.fromTransient);
 ...
 * limitations under the License.
 */
const toLedgerContractState = (contractState) => ledgerV6.ContractState.deserialize(contractState.serialize());
const fromLedgerContractState = (contractState) => compactRuntime.ContractState.deserialize(contractState.serialize());
const toLedgerQueryContext = (queryContext) => {
    const stateValue = ledgerV6.StateValue.decode(queryContext.state.state.encode());
    const ledgerQueryContext = new ledgerV6.QueryContext(new ledgerV6.ChargedState(stateValue), queryContext.address);
...
const contractMaintenanceAuthority = (sk, contractState) => {
    ...
    const threshold = 1;
    return new ledgerV6.ContractMaintenanceAuthority([svk], threshold, contractState ? contractState.maintenanceAuthority.counter + 1n : 0n);
};
...
const createUnprovenLedgerDeployTx = (verifierKeys, sk, contractState, zswapLocalState, encryptionPublicKey) => {
    ...
    const contractDeploy = new ledgerV6.ContractDeploy(ledgerContractState);
    return [
        contractDeploy.address,
        fromLedgerContractState(contractDeploy.initialState),
        midnightJsTypes.Transaction.fromParts(midnightJsNetworkId.getNetworkId(), zswapStateToOffer(zswapLocalState, encryptionPublicKey), undefined, ledgerV6.Intent.new(midnightJsUtils.ttlOneHour()).addDeploy(contractDeploy))
    ];
};
const createUnprovenLedgerCallTx = (circuitId, contractAddress, initialContractState, zswapChainState, partitionedTranscript, privateTranscriptOutputs, input, output, nextZswapLocalState, encryptionPublicKey) => {
    ...
    return midnightJsTypes.Transaction.fromParts(midnightJsNetworkId.getNetworkId(), zswapStateToOffer(nextZswapLocalState, encryptionPublicKey, {
        contractAddress,
        zswapChainState
    }), undefined, ledgerV6.Intent.new(midnightJsUtils.ttlOneHour()).addCall(new ledgerV6.ContractCallPrototype(contractAddress, circuitId, op, partitionedTranscript[0], partitionedTranscript[1], privateTranscriptOutputs, input, output, ledgerV6.communicationCommitmentRandomness(), circuitId)));
};
// Utilities for creating single contract updates.
const replaceAuthority = (newAuthority, contractState) => new ledgerV6.ReplaceAuthority(contractMaintenanceAuthority(newAuthority, contractState));
const removeVerifierKey = (operation) => new ledgerV6.VerifierKeyRemove(operation, new ledgerV6.ContractOperationVersion('v2'));
const insertVerifierKey = (operation, newVk) => new ledgerV6.VerifierKeyInsert(operation, new ledgerV6.ContractOperationVersionedVerifierKey('v2', newVk));
...
const unprovenTxFromContractUpdates = (contractAddress, updates, contractState, sk) => {
    const maintenanceUpdate = new ledgerV6.MaintenanceUpdate(contractAddress, updates, contractState.maintenanceAuthority.counter);
    // 'idx' is '0n' because Midnight.js currently only supports single-party maintenance update authorities
    const idx = 0n;
    const signedMaintenanceUpdate = maintenanceUpdate.addSignature(idx, ledgerV6.signData(sk, maintenanceUpdate.dataToSign));
    return midnightJsTypes.Transaction.fromParts(midnightJsNetworkId.getNetworkId(), undefined, undefined, ledgerV6.Intent.new(midnightJsUtils.ttlOneHour()).addMaintenanceUpdate(signedMaintenanceUpdate));
};
...
const partitionTranscript = (initialTxContext, finalTxContext, publicTranscript) => {
    const partitionedTranscripts = ledgerV6.partitionTranscripts([
        new ledgerV6.PreTranscript(Array.from(finalTxContext.comIndices).reduce((queryContext, entry) => queryContext.insertCommitment(...entry), toLedgerQueryContext(initialTxContext)), publicTranscript)
    ], ledgerV6.LedgerParameters.initialParameters());
...
const call = (options) => {
    ...
    const { result, context, proofData } = circuit({
        ...
        currentZswapLocalState: compactRuntime.emptyZswapLocalState(midnightJsUtils.parseCoinPublicKeyToHex(coinPublicKey, midnightJsNetworkId.getNetworkId())),
...
const callContractConstructor = (options) => {
    const constructorResult = options.contract.initialState({
        ...
        initialZswapLocalState: compactRuntime.emptyZswapLocalState(options.coinPublicKey)
    }, ...('args' in options ? options.args : []));
    return {
...
const submitTx = async (providers, options) => {
    ...
    const provenTx = await providers.proofProvider.proveTx(recipe.transaction, proveTxConfig);
    ...
    if (process.env.MN_DEBUG) {
        ...
        if (options.circuitId) {
            const vtx = bound.wellFormed(ledgerV6.LedgerState.blank(midnightJsNetworkId.getNetworkId()), new ledgerV6.WellFormedStrictness(), new Date(Date.now()));
            console.log(`Vtx: ${vtx}`);
        }
    }
...
async function createUnprovenDeployTx(providers, options) {
    ...
    return createUnprovenDeployTxFromVerifierKeys(verifierKeys, midnightJsUtils.parseCoinPublicKeyToHex(providers.walletProvider.zswapSecretKeys.coinPublicKey, midnightJsNetworkId.getNetworkId()), options, providers.walletProvider.zswapSecretKeys.encryptionPublicKey);
}
...
function createUnprovenCallTxFromInitialStates(options, walletCoinPublicKey, walletEncryptionPublicKey) {
    ...
    return {
        ...
        private: {
            ...
            newCoins: zswapStateToNewCoins(midnightJsUtils.parseCoinPublicKeyToHex(coinPublicKey, midnightJsNetworkId.getNetworkId()), callResult.private.nextZswapLocalState)
        }
    };
}
...
async function createUnprovenCallTx(providers, options) {
    ...
    if (hasPrivateStateId && hasPrivateStateProvider) {
        ...
        return createUnprovenCallTxFromInitialStates(createCallOptions(options, midnightJsUtils.parseCoinPublicKeyToHex(providers.walletProvider.zswapSecretKeys.coinPublicKey, midnightJsNetworkId.getNetworkId()), contractState, zswapChainState, privateState), providers.walletProvider.zswapSecretKeys.coinPublicKey, providers.walletProvider.zswapSecretKeys.encryptionPublicKey);
    }
    const { zswapChainState, contractState } = await getPublicStates(providers.publicDataProvider, options.contractAddress);
    return createUnprovenCallTxFromInitialStates(createCallOptions(options, midnightJsUtils.parseCoinPublicKeyToHex(providers.walletProvider.zswapSecretKeys.coinPublicKey, midnightJsNetworkId.getNetworkId()), contractState, zswapChainState), providers.walletProvider.zswapSecretKeys.coinPublicKey, providers.walletProvider.zswapSecretKeys.encryptionPublicKey);
}
