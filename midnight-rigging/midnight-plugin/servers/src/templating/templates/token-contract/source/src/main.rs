...
/// Lightweight CLI tool for interacting with a fungible **{{contract_name}}** contract that follows the
/// standard `balanceOf`, `transfer`, and `Transfer`-event APIs. All calls are executed against a
/// Pod JSON-RPC node provided via the `--rpc-url` flag (defaults to `{{default_rpc_url}}`).
#[derive(Parser)]
#[command(author, version, about)]
struct Cli {
    /// Deployed address of the **{{contract_name}}** contract to interact with.
...
async fn watch(
    ...
    let mut filter = LogFilterBuilder::new()
        ...
        .event_signature({{contract_name}}::Transfer::SIGNATURE_HASH);
    ...
    while let Some(log) = stream.next().await {
        ...
        let event = {{contract_name}}::Transfer::decode_log(&log.inner.inner)
...
async fn address_balance(
    ...
    let pod_provider = PodProviderBuilder::with_recommended_settings()
        .on_url(&rpc_url)
        .await?;
    let tokens = {{contract_name}}::new(contract_address, pod_provider.clone());
