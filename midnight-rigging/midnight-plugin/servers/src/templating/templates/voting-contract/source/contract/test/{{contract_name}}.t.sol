...
/**
 * {{contract_name}} Test Suite
 *
 * Generated by: {{TEMPLATE_NAME}} v{{TEMPLATE_VERSION}}
 * Date: {{CURRENT_DATE}}
 */
contract {{contract_name}}Test is PodTest {
    ...
    function test_createProposal() public {
        ...
        emit {{contract_name}}.ProposalCreated(bytes32(0), Time.currentTime().addSeconds(1), "test");
    ...
    function test_singleVoterProposal() public {
        ...
        vm.startBroadcast(users[0]);
        vm.expectEmit();
        emit {{contract_name}}.VoteCast(id, users[0], 1);
        voting.castVote(id, 1);

        podWarp(Time.currentTime().addSeconds(2));
        vm.expectEmit(true, true, false, true);
        emit {{contract_name}}.ProposalExecuted(id);
        voting.execute(id);
    }
    ...
    function test_voteAndPickWinner() public {
        ...
        for (uint256 i = 0; i < users.length; i++) {
            ...
            vm.expectEmit();
            emit {{contract_name}}.VoteCast(id, users[i], 1);
