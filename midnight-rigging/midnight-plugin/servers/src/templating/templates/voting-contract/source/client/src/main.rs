...
use {{bindings_crate_name}}::{{contract_name | snakecase}}::{{contract_name}};

/// {{contract_name}} CLI Client
///
...
struct Cli {
    /// Address of the {{contract_name}} contract
    #[arg(long)]
...
async fn vote(
    ...
    let pod_provider = PodProviderBuilder::with_recommended_settings()
        ...
        .on_url(&rpc_url)
        .await?;
    let voting = {{contract_name}}::new(contract_address, pod_provider);
...
async fn get_votes(
    ...
    contract_address: Address,
    proposal_id: ProposalId,
) -> Result<Vec<{{contract_name}}::VoteCast>> {
    let pod_provider = PodProviderBuilder::with_recommended_settings()
        .on_url(&rpc_url)
        .await?;
    ...
    let filter = LogFilterBuilder::new()
        ...
        .event_signature({{contract_name}}::VoteCast::SIGNATURE_HASH)
    ...
    for log in logs {
        let vote = {{contract_name}}::VoteCast::decode_log_data(log.data())?;
        votes.push(vote);
    }
...
async fn create_proposal(
    ...
    let pod_provider = PodProviderBuilder::with_recommended_settings()
        ...
        .on_url(&rpc_url)
        .await?;
    let voting = {{contract_name}}::new(contract_address, pod_provider.clone());
    ...
    let proposal_created = {{contract_name}}::ProposalCreated::decode_log_data(event.data())?;
...
async fn watch(rpc_url: String, contract_address: Address, proposal_id: ProposalId) -> Result<()> {
    ...
    while let Some(log) = stream.next().await {
        ...
        let event = {{contract_name}}::{{contract_name}}Events::decode_log(&log.inner.inner)
            .context("decoding event failed. deployed contract version might not match")?;
        if let {{contract_name}}::{{contract_name}}Events::VoteCast(vote) = event.data {
            println!(
                "Voter {} voted {}, tx: {}",
                vote.voter,
                vote.choice,
