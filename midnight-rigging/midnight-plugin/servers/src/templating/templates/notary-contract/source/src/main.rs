...
async fn watch(
    ...
    let mut filter = LogFilterBuilder::new()
        ...
        .event_signature({{contract_name}}::DocumentTimestamped::SIGNATURE_HASH);
    ...
    while let Some(log) = stream.next().await {
        ...
        let event = {{contract_name}}::DocumentTimestamped::decode_log(&log.inner.inner)
...
async fn timestamp(
    ...
    let pod_provider = PodProviderBuilder::with_recommended_settings()
        ...
        .on_url(&rpc_url)
        .await?;
    let notary = {{contract_name}}::new(contract_address, pod_provider.clone());
...
async fn get_timestamp(
    ...
    let notary = {{contract_name}}::new(contract_address, pod_provider);
