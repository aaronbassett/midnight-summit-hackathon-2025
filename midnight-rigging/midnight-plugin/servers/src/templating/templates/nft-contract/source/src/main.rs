...
/// Simple CLI for interacting with the {{contract_name}} contract.
#[derive(Parser)]
#[command(author, version, about, long_about = None)]
struct Cli {
    /// Deployed address of the {{contract_name}} contract.
...
async fn watch(
    ...
    let mut filter = LogFilterBuilder::new()
        ...
        .event_signature({{contract_name}}::Minted::SIGNATURE_HASH);
    ...
    while let Some(log) = stream.next().await {
        ...
        let event = {{contract_name}}::Minted::decode_log(&log.inner.inner)
...
async fn mint(
    ...
    let pod_provider = PodProviderBuilder::with_recommended_settings()
        .on_url(&rpc_url)
        .await?;
    let nfts = {{contract_name}}::new(contract_address, pod_provider.clone());
